...
export default function DistributePage() {
  const [evalType, setEvalType] = useState<EvalType>('offline')
  const [curriculum, setCurriculum] = useState<Curriculum>('2022')
  const [schoolLevel, setSchoolLevel] = useState<SchoolLevel>('high')
  const [subject, setSubject] = useState<string>('국어')
  ...
  const selectedStandard = useMemo(() =>
    SAMPLE_ACHIEVEMENT_STANDARDS.find(s => s.code === selectedStandardCode) || null
          ...
          <div className="flex-1 flex flex-col gap-2">
            ...
            <div className="relative">
              <select
                ...
                className="w-full border border-[#DDDDDD] rounded-[4px] p-3 text-[16px] font-bold text-[#2B2B2B] bg-white appearance-none cursor-pointer"
              >
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="relative">
              <select
                ...
                className="w-full border border-[#DDDDDD] rounded-[4px] px-3 py-3 text-[16px] font-bold text-[#2B2B2B] bg-white appearance-none cursor-pointer"
              >
                <option value="">성취기준을 선택하세요</option>
                {filteredStandards.map(s => (
                  <option key={s.code} value={s.code}>{s.code} {s.content}</option>
                ))}
              </select>
              <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[#808080] pointer-events-none">▾</span>
            </div>
